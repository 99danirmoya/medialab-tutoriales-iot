...
void setup(){
  ...
  // Inicializacion pines de perifericos que envian por LoRa ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  pinMode(VBAT_PIN, INPUT);                                              // VBAT_PIN como input. Es el "sensor" de voltaje de la batería
  pinMode(VOUT_PIN, OUTPUT);                                             // Pin para alimentar al sensor por medio de un MOSFET cuya gate se controla por este pin
  pinMode(TRIG_PIN, OUTPUT);                                             // TRIG_PIN (Rx) como output. Inicia los pulsos ultrasonicos cuando se pone en 'HIGH' 
  pinMode(ECHO_PIN, INPUT);                                              // ECHO_PIN (Tx)

  digitalWrite(VOUT_PIN, LOW);                                           // La alimentación del sensor se desconecta inicialmente para activarse cuando sea necesario
