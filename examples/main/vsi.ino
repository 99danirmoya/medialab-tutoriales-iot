...
void carga_valores(uint16_t distancia_nueva){
  for(int i = TARGET_ARRAY_LENGTH - 1; i > 0; i--){          // El indice del bucle va desde la última posición del array a la primera
  ...
  bufferCircular[0] = distancia_nueva;                       // Cada vez que quiero meter un valor nuevo, lo cargo en la primera posición del array
}
...
uint32_t variable_send_interval(){
  ...
  uint32_t VARIABLE_SEND_INTERVAL = (desviacion_estandar_lista_medidas_medias <= 1) ? SEND_INTERVAL_RELAXED : SEND_INTERVAL_INTENSIVE;

  Serial.print("El send interval es: "); Serial.println(VARIABLE_SEND_INTERVAL); Serial.println(F("==================="));
  return VARIABLE_SEND_INTERVAL;
}
...
uint32_t static_send_interval(){
  uint32_t SEND_INTERVAL = SEND_INTERVAL_STATIC;

  Serial.print("El send interval es: "); Serial.println(SEND_INTERVAL); Serial.println(F("==================="));
